...
int main( void )
{
	...
	while( ( opcion = opcionMenu() ) != '6' )
	{
		switch( opcion )
		{
			case '1':
				cout << endl;
				if( cola.obtenerTamanio() != TAM )
				{
					cout << "Nombre del documento a encolar: ";
					cin.ignore();
			...
			case '2':
				if( cola.obtenerTamanio() > 0 )
					...
					cout << "La cola de impresion esta vacia" << endl << endl;
				break;
			case '3':
				if( cola.obtenerTamanio() > 0 )
...
template < typename E >
void mostrar( ColaEstatica< E > &cola )
{
